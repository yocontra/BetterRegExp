...
describe 'constructor()', ->
  ...
  it 'should create from itself', (done) ->
    pat = new BetterRegExp new BetterRegExp /^\s*\d+\s*$/
    done()
  it 'should create from itself with flags', (done) ->
    pat = new BetterRegExp new BetterRegExp /^\s*\d+\s*$/g
    done()
...
describe 'isX()', ->
  it 'should work', (done) ->
    for flag, name of util.flags
      pat = new BetterRegExp /^\s*\d+\s*$/, flag
...
describe 'X()', ->
  it 'should work', (done) ->
    for flag, name of util.flags
      pat = new BetterRegExp(/^\s*\d+\s*$/)[flag]()
      pat["is#{util.uppercase(name)}"]().should.equal true
      pat = new BetterRegExp(/^\s*\d+\s*$/)[name]()
      pat["is#{util.uppercase(name)}"]().should.equal true
    done()
